...
// Preset tracking parameters for major analytics platforms
const trackingPresets = {
  ...
  meta: [
    ...
    { key: 'utm_medium', value: 'social' },
    { key: 'utm_campaign', value: 'qr_code' }
  ],
  adobe: [
    { key: 'cid', value: 'qr_code' },
    { key: 's_kwcid', value: '' }
  ]
}

const addTrackingPreset = (platform: 'google' | 'meta' | 'adobe') => {
  const preset = trackingPresets[platform]
  ...
  <div class="container mx-auto p-8 max-w-xl">
    ...
    <Head>
      ...
      <Meta name="description"
        content="Generate QR codes instantly with customizable settings. Download as PNG or SVG with no signup required." />
    ...
    <div class="space-y-6">
        ...
        <transition enter-active-class="transition-all duration-300 ease-out"
          ...
          <div class="space-y-3 overflow-hidden">
            ...
            <div class="pt-2 border-t">
              <div class="text-xs text-muted-foreground mb-2">Quick add tracking params:</div>
              <div class="flex gap-2 flex-wrap">
                <Button @click="addTrackingPreset('google')" variant="secondary" size="sm">
                ...
                <Button @click="addTrackingPreset('meta')" variant="secondary" size="sm">
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="w-4 h-4 mr-1">
                    <path fill="#1877F2" d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
                  </svg>
                  Meta
                </Button>
                <Button @click="addTrackingPreset('adobe')" variant="secondary" size="sm">
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="w-4 h-4 mr-1">
      ...
      <div class="flex space-x-2">
        <Button @click="generateQR" :disabled="!url || isGenerating" class="flex-1 relative group overflow-hidden">
          ...
          <span class="absolute bottom-0 left-0 h-1 bg-primary/50 transition-all duration-300 ease-out"
            :class="isGenerating ? 'w-full' : 'w-0'"></span>
        </Button>
      ...
      <div v-if="qrCode || isGenerating" class="space-y-4">
        <div class="flex justify-center p-4 bg-white rounded-lg border">
          ...
          <img v-if="qrCode" :src="qrCode" alt="QR Code" class="max-w-full animate-fadeIn" />

          <!-- Loading placeholder -->
          <div v-if="isGenerating && !qrCode"
        ...
        <div class="flex flex-wrap gap-2">
          ...
          <Button v-if="isSharingSupported" @click="shareQRCode" variant="outline" class="flex-1">
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
              stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
              class="w-4 h-4 mr-2">
    ...
    <!-- History Panel -->
    <transition enter-active-class="transition-all duration-300 ease-out"
      leave-active-class="transition-all duration-200 ease-in" enter-from-class="opacity-0" enter-to-class="opacity-100"
